...
/**
 * <p>
 * A {@code TypeMapper} allows for interaction with the SQL -&gt; Java mapping
 * process. Types in EoD SQL are mapped based on what the Java type is, not what
 * the SQL type is, this allows for greater flexibility since many Java types
 * can all map to the same SQL type.
 * </p><p>
 * By default most data-types you will need already have
 * <code>TypeMapper</code>s:
 * </p>
 * <ul>
 * <li>String</li>
 * <li>Byte / byte</li>
...
public interface TypeMapper<T> {
   ...
   * <code>ResultSet</code> into the type this <code>TypeMapper</code> returns.
   *
   ...
   * @throws SQLException if the SQL type cannot be mapped with this
   * <code>TypeMapper</code>
   */
  T get( ResultSet results, int column ) throws SQLException;

  /**
   * Attempts to convert the spcified object into a SQL object, and place it
   * into a <code>ResultSet</code>. This method is used by
   ...
   * @param column the column in the <code>ResultSet</code> where the data
   * should be put
  ...
  /**
   * Attempts to convert the specified object into a SQL object and place it
   * into the <code>PreparedStatement</code>.
   *
